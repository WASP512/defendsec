...
def disk_encryption() -> bool | None:
    ...
    if system == "Darwin":
        ...
        return None
    if system == "Linux":
        raw = run(["lsblk", "-o", "TYPE,MOUNTPOINT"])
        if "crypt" in raw:
            return True
...
def main() -> int:
    ...
    if not node_key:
        ...
        try:
            enrolled = post_json(
                f"{server}/api/enroll",
                {"enrollSecret": args.enroll_secret, "hostname": socket.gethostname()},
            )
        except urllib.error.HTTPError as exc:
            print(f"Enroll failed: {exc.read().decode('utf-8', errors='replace')}", file=sys.stderr)
            return 1
        node_key = enrolled["nodeKey"]
        state.write_text(node_key, encoding="utf-8")
        print(f"Enrolled as {enrolled.get('deviceId')}")
    ...
    while True:
        ...
        try:
            ...
            print(f"Check-in failed: {exc.read().decode('utf-8', errors='replace')}", file=sys.stderr)
            return 1
